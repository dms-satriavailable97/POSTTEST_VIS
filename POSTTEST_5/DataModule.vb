...
Module DataModule
    ...
    Public Function TampilJoki() As DataTable
        Dim dt As New DataTable()
        Try
            Dim query As String = "SELECT j.uid, j.username, j.password, j.detail, l.nama_layanan, j.kesulitan, j.total_harga " &
                             "FROM tb_joki j JOIN tb_layanan l ON j.id_layanan = l.id_layanan"
            Using conn As MySqlConnection = GetConnection()
                Using da As New MySqlDataAdapter(query, conn)
                    da.Fill(dt)
                End Using
            End Using
        Catch ex As Exception
            MessageBox.Show("Gagal tampil data joki: " & ex.Message)
        End Try
        Return dt
    End Function
    ...
    Public Function CariJoki(keyword As String) As DataTable
        ...
        Try
            Dim query As String = "SELECT j.uid, j.username, j.password,j.detail, l.nama_layanan, j.kesulitan, j.total_harga " &
                             "FROM tb_joki j JOIN tb_layanan l ON j.id_layanan = l.id_layanan " &
                             "WHERE j.uid LIKE @key OR j.username LIKE @key OR l.nama_layanan LIKE @key OR j.detail LIKE @key"
    ...
    Public Function UbahJoki(uid As String, user As String, pass As String, id_lay As String, sulit As Integer, total As Integer) As Boolean
        Try
            Dim query As String = "UPDATE tb_joki SET username=@user, password=@pass, id_layanan=@id_lay, kesulitan=@sulit, total_harga=@total WHERE uid=@uid"
            Using conn As MySqlConnection = GetConnection()
                conn.Open()
                Using cmd As New MySqlCommand(query, conn)
                    cmd.Parameters.AddWithValue("@uid", uid)
    ...
    Public Function HapusLayanan(id As String) As Boolean
        Try
            ...
            Return True
        Catch ex As Exception
            MessageBox.Show("Gagal hapus layanan: " & ex.Message & ". Pastikan layanan ini tidak sedang digunakan di antrean joki.")
            Return False
        End Try
    End Function
